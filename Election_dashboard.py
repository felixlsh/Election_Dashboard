...
# 2. [데이터 로드 엔진] 저장된 최종 CSV 파일을 읽어옵니다.
...
if not df_history.empty:
    ...
    # 판다스에서 역전 데이터만 안전하게 필터링한 뒤 Altair에 전달
    ...
    # 최신 순 정렬 및 혹시 모를 개표율 열 강제 은닉 처리
    df_display = df_history[::-1].copy().drop(columns=['개표율'], errors='ignore')
